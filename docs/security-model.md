...
All owner-run workflows require the local `AZURE_SUBSCRIPTION_ID`, verify it without displaying account metadata, set it as the Azure CLI active subscription, and confirm the resulting context. Subscription- and resource-bound commands receive `--subscription` explicitly. Tenant-bound `az ad signed-in-user show` does not; it runs only after the active subscription is established. The Bicep reader object ID is not stored in the parameter file: `what-if` and deployment resolve the current signed-in user and supply it as an in-memory override. Bootstrap and final validation resolve the same current user through the same context sequence.
