...
| Local developer identity | Yes when local authentication is configured | Depends on assigned app role | Owner-run deployment, bootstrap, and validation only | Temporary `Key Vault Secrets Officer`, then final `Key Vault Secrets User`, both at vault scope | Initial infrastructure deployment and negative validation succeeded; bootstrap and final validation remain pending. |
