...
The repository owner must privately select and verify the intended Azure CLI active subscription before running any workflow. Each script captures that active subscription once without displaying account metadata and never changes the CLI context. Subscription- and resource-bound commands receive the captured value through `--subscription`. Tenant-bound `az ad signed-in-user show` does not; it runs only after the active context has been captured. The Bicep reader object ID is not stored in the parameter file: `what-if` and deployment resolve the current signed-in user and supply it as an in-memory override. Bootstrap and final validation resolve the same current user under that already established context.

Direct role assignments are not interchangeable with inherited effective permissions. Direct validation globally enumerates all assignments for the vault scope with principal-name filling disabled, then filters exact scope locally. This global view detects any direct Officer or application-identity assignment introduced at the vault. Effective inherited validation is separately constrained to the current development user and its transitive groups. An effective inherited role with Key Vault data actions invalidates the development least-privilege assumptions and causes sanitized failure as an unsupported precondition. Unrelated inherited assignments for other principals are excluded because they are not effective permissions of the development user.
