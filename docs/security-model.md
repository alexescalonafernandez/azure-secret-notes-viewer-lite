...
The owner-run preflight succeeded, but the first attempted `what-if` exposed the former linked-parameter incompatibility before any Azure resource deployment occurred. Scripts 01 and 02 preserve Azure CLI diagnostics in the private local terminal for investigation. Shared security evidence remains limited to sanitized conclusions and markers; raw diagnostics, identifiers, URLs, principal data, and resource names must not be published.
