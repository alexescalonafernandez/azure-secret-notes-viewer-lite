...
Every workflow requires `AZURE_SUBSCRIPTION_ID`, verifies it without printing account data, sets it as the Azure CLI active subscription, and confirms the resulting context. Subscription- and resource-bound commands still receive `--subscription` explicitly. The tenant-bound `az ad signed-in-user show` command does not support that option, so it runs only after the active subscription has been established. The `what-if` and deployment workflows inject the resolved current user's object ID into the existing Bicep parameter locally. Bootstrap and validation resolve the same current user through the same context sequence, preventing a separately configured reader principal from drifting away from the operator identity.
