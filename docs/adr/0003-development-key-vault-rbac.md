...
Require `AZURE_SUBSCRIPTION_ID` for every owner-run workflow. Verify it without printing it, set it as the Azure CLI active subscription, and confirm that context. Pass `--subscription` explicitly to subscription- and resource-bound operations. Resolve the current signed-in user only after that setup; the tenant-bound `az ad signed-in-user show` command does not receive the unsupported subscription option. Supply that user's object ID as an in-memory Bicep override rather than storing a separate principal in the local parameter file.
