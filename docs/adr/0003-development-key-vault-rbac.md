...
Require the repository owner to privately select and verify the intended Azure CLI active subscription before execution. Each script captures that active subscription once without printing it and does not change the CLI context. Pass the captured subscription explicitly to subscription- and resource-bound operations. Resolve the current signed-in user only after the capture; the tenant-bound `az ad signed-in-user show` command does not receive the unsupported subscription option. Supply that user's object ID as an in-memory Bicep override rather than storing a separate principal in the local parameter file.
